...
- **Body** (400, 1rem, 1.6): the page default. Long descriptions cap at `65ch`, except Read-mode prose on `/projects/[slug]` (case-study and short-page body copy), which is held to a `36rem` measure (about 70 characters of Recursive at 1rem; a `ch` cap measures the wide "0", so `68ch` ran to about 87).
...
`/projects/[slug]` is Read mode: the board and the panels are for scanning, this is where a visitor reads. Every page shares a header and a screenshot pair, then branches into a flagship's case study or a short page built only from `lib/data.ts`. A short page shows the screenshots straight after the header; a flagship leads with "The brief" and shows them after it, full grid width, before "What I built", so the first screen is about the work, not the client's brand.

- **Header** (`ProjectHeader`): "← All projects", an eyebrow ("// case study" or "// project"), the H1 title beside `StatusBadge`, the one-line summary, a flagship's role on its own line in sans `text-base` `ink` (the fact a hiring reader came for), a mono `band · sector` line, a mono meta strip (dates, payment gateways) where each applies, and "Open live site" when `canLinkLive` allows it. "Open live site" is a secondary button (it leaves the site), so the page's one amber primary is "Start a project".
- **Screenshots** (`ProjectScreenshots`): the desktop and mobile pair inside `ImageLightbox`, the same component the field log uses. Below 640px only the desktop shot shows; the phone shot would repeat it. A project with no capture shows a plain panel naming why (`caseStudyContent.noPreview`): behind a login, an internal system, or simply not captured yet.
- **Short page** (`ProjectSummary`, ten of the fifteen projects): "About the project" (`description`, then `contribution` if the project has one) and "Stack" as a chip list. Nothing invented; every line already lived in `lib/data.ts`. A project that a case study names in `related` (the BeachBus NFC system) adds one `ink` link under About, "Part of the <title> case study" (`caseStudyContent.partOf`), so the fuller story is one click away.
- **Flagship body** (`CaseStudyBody`, the five researched case studies): "The brief", "What I built", "Decisions" (numbered, each "chose X over Y because Z"), "Stack" (a name beside one line of why), "Outcome" (qualitative, then any client-approved metrics as numeral cells), and "From the client" when a recommendation's `projectId` matches. A study with `related` work ends its column with a hairline-topped "Related" line: a mono `muted-strong` label beside each related project's title as an `ink` link. A footer links to the next case study in reading order, then repeats the "Start a project" / "All projects" pair.

- **404** (`app/not-found.tsx`): every unknown address, including an unknown project slug, on the one grid: eyebrow, `.text-fluid-h1`, one sentence (`notFoundContent`), then "All projects" as the amber primary and "Back to the homepage" as the mono secondary.

**The Read-Mode Measure.** Both the short page and the flagship body hold their prose to a `max-w-[36rem]` column, about 70 characters a line at the body size. A `ch` cap does not hold in Recursive: `ch` is the width of its wide "0", so the earlier `68ch` column ran to about 87 characters. Every section heading in both, "About the project" through "From the client", uses the Title tier (`text-xl font-semibold`), one step below `.text-fluid-h2`, because it is a heading inside a page whose H1 already carries that weight.
